...
from get_kg_actions_for_gui import KGActionStore, open_gds, test_dir, get_user_input_prompt
from get_kg_actions_for_gui import addl_action_list, end_differential_actions, end_initial_assessment_actions
from get_kg_actions_for_gui import actions_vectorstore
...
# (11) interactive LLM workflow that pauses for user-centered medical results
# instead of running every action immediately, this file returns the next test/action and a prompt dictionary telling the GUI what info to collect
...
def get_stage_response_interactive(current_model, input_type, ehr_token, new_stage = 'Initial Assessment Stage', user_input = {}, medical_info_cache=None):	
	if medical_info_cache is None:
		medical_info_cache = {}
	# Load EHR data if model run
	interactive_multi_stages = ['Initial Assessment Stage', 'Differential Diagnosis Stage']
	interactive_single_stages = ['Monitoring Stage']
	interactive_stages = interactive_multi_stages+interactive_single_stages
	#
	cm_medical_info, action_store = load_medical_info_and_action_store(current_model, input_type, ehr_token, user_input, medical_info_cache)	
	cm_medical_info.seen_stages.append('%s %s'% (cm_medical_info.get_current_time(), new_stage))
	#
	# inital assessment and differential have multiple stages
	print(new_stage)
	next_step, prompt_dict = 'next', {}
	if '|' in new_stage:
		new_stage, next_step = new_stage.split('|')
	#
	if new_stage == 'Initial Assessment Stage':
			cm_medical_info = action_store.get_results(cm_medical_info, next_step)
			cm_medical_info, next_step = make_initial_assesement(current_model, cm_medical_info, cm_medical_info.potential_disease_str, action_store)
		...
		cm_medical_info = get_differential_result(current_model, cm_medical_info, next_step, action_store)
		cm_medical_info, next_step = get_differential_diagnosis(current_model, cm_medical_info, action_store)
		...
		if next_step == 'next':
			...
			if new_stage in interactive_stages:
				...
				if new_stage == 'Initial Assessment Stage':
					...
					cm_medical_info, next_step = monitor_crew_member(current_model, cm_medical_info, action_store)
				#
		cm_medical_info.seen_stages.append('%s %s'% (cm_medical_info.get_current_time(), new_stage))
		#
		if next_step != 'next':
			prompt_dict = get_user_input_prompt(cm_medical_info, next_step, new_stage)
			new_stage = '|'.join([new_stage, next_step])
	#
	if new_stage == 'Stable Health Stage':
		store_medical_info(cm_medical_info, ehr_token, medical_info_cache, delete=True)
	else:
		store_medical_info(cm_medical_info, ehr_token, medical_info_cache)
	return cm_medical_info, new_stage, prompt_dict

def load_medical_info_and_action_store(current_model, input_type, ehr_token, user_input, medical_info_cache=None):
	if medical_info_cache is None:
		medical_info_cache = {}
	...
	if ehr_token in medical_info_cache:
		cm_medical_info = medical_info_cache[ehr_token]
	else:
		...
		cm_medical_info.interactive_result = user_input
		cm_medical_info = action_store.run_survey(cm_medical_info, 'tests/'+test_dir)
	cm_medical_info.interactive_result = user_input
	return cm_medical_info, action_store

def store_medical_info(cm_medical_info, ehr_token, medical_info_cache=None, delete=False):
	if medical_info_cache is None:
		medical_info_cache = {}
	if delete:
		medical_info_cache.pop(ehr_token, None)
	else:
		medical_info_cache[ehr_token] = cm_medical_info
...
def make_initial_assesement(current_model, cm_medical_info, potential_disease_str, action_store):
	...
	if len(possible_actions) > 0:
		...
		if next_step not in end_initial_assessment_actions:
			...
			next_step = 'next'
	return cm_medical_info, next_step
...
def get_differential_result(current_model, cm_medical_info, next_step, action_store):
	cm_medical_info = action_store.get_results(cm_medical_info, next_step)
