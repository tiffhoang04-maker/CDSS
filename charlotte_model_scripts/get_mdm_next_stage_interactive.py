...
# (10) interactive versio of the MDM stage loop
# splits stages into smaller pieces so the UI can ask the user for test results before continuing
...
class GetInteractiveNextStageHandler(object):
	...
	def on_get(self, req, resp, input_type:str, ehr_token:str, new_stage:str, user_inputs:dict):
		...
		print('get')
		print(user_inputs)
		user_input = user_inputs.json()
		if "user_input" in user_inputs_dict:
			user_input = user_input["user_input"]
		print(user_input)
		#
		cm_medical_info, new_stage, prompt_dict = get_stage_response_interactive(self.current_model, input_type, ehr_token, new_stage, user_input, self.medical_info_cache)
		next_test = 'next'
		if '|' in new_stage:
