...
# (3) implements the medical stage logic and LLM prompting (most of the clinical reasoning workflow)
# builds prompts 
# ask LLM for next actions, diagnoses, severity labels, prescriptions, and monitoring choices
# uses FAISS (i think FAISS was something Walt and I added while trying to maneuver around the dependencies; not sure if C.N. originally had FAISS in her code)
# to map LLM text back to known options 
...
def get_stage_response(current_model, input_type, ehr_token, new_stage = 'Initial Assessment Stage', medical_info_cache=None):	
	if medical_info_cache is None:
		medical_info_cache = {}
	# Load EHR data if model run
	cm_medical_info, action_store = load_medical_info_and_action_store(current_model, input_type, ehr_token, medical_info_cache)	
...
def load_medical_info_and_action_store(current_model, input_type, ehr_token, medical_info_cache=None):
	if medical_info_cache is None:
		medical_info_cache = {}
